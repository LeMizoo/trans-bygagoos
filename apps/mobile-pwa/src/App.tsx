...
function AppContent() {
  ...
  return (
    <>
      ...
      <button className="fab-refresh" onClick={() => window.location.reload()} title="Actualiser">🔄</button>
      <button className="fab-assistance" onClick={() => alert('📞 Assistance : contactez l\'administration')} title="Aide">🆘</button>
    </>
  );
}
...
function LoginPage({ onLogin }: { onLogin: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onLogin();
    } catch { setError('Code introuvable'); } finally { setLoading(false); }
  };
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0a', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 20, fontFamily: 'system-ui, sans-serif' }}>
      <div className="app-header" style={{ position: 'fixed', top: 0 }}>
        <div className="header-content">
          <div className="header-left"><div className="header-logo"><img src="/assets/logo/b-trans.png" alt="Logo" /></div><div className="header-info"><h1>Trans ByGagoos</h1><p>Application Chauffeur</p></div></div>
        </div>
      </div>
...
function Header({ onLogout }: { onLogout: () => void }) {
  ...
  useEffect(() => {
    const go = () => setOnline(true); const goff = () => setOnline(false);
    ...
    <div className="app-header">
      <div className="header-content">
        ...
        <div className="header-right">
          <button className="icon-btn sync" onClick={() => window.location.reload()} title="Synchroniser">🔄</button>
          <button className="icon-btn" onClick={() => alert('📬 Notifications')} title="Notifications">🔔</button>
          <button className="icon-btn logout" onClick={onLogout} title="Déconnexion">🚪</button>
          <button className={`icon-btn ${online ? 'online' : 'offline'}`} title={online ? 'En ligne' : 'Hors ligne'}>{online ? '📶' : '📡'}</button>
        </div>
      </div>
    </div>
  );
...
function DashboardPage() {
  ...
  const pointer = useMutation({
    ...
    onSuccess: (_, type) => {
      ...
      setMsg(labels[type] || '✅ OK'); queryClient.invalidateQueries({ queryKey: ['dashboard'] }); setTimeout(() => setMsg(''), 3000);
    },
  });
  ...
  const handleCourse = () => {
    if (typeCourse === 'NORMALE') { const d = parseFloat(kmArrivee) - parseFloat(kmDepart); if (d <= 0) { setMsg('⚠️ Km arrivée > Km départ'); return; } createCourse.mutate({ chauffeurId: c?.id, motoId: m?.id, type: 'NORMALE', distance: d }); }
  ...
  return (
    <div>
      ...
      <div className="card"><div className="card-title">➕ Nouvelle course</div><div className="form-group"><select value={typeCourse} onChange={e => setTypeCourse(e.target.value)}><option value="NORMALE">🚖 Course normale</option><option value="ADY_VAROTRA">🛺 Ady Varotra</option><option value="LOCATION_JOURNALIERE">📅 Location journalière</option></select></div>
        ...
        <button onClick={handleCourse} disabled={createCourse.isPending} className="btn-primary">{createCourse.isPending ? '⏳...' : '✅ Enregistrer'}</button></div>
...
function CoursesPage() {
  ...
  return <div><div className="card"><div className="card-title">📋 Mes courses</div></div>{courses.length === 0 ? <p style={{ color: '#888', textAlign: 'center', padding: 20 }}>Aucune course</p> : courses.slice(0, 50).map((course: any) => <div key={course.id} className="course-item"><div><div style={{ fontWeight: 'bold', color: '#DAA520', fontSize: 12 }}>{course.type}</div><div className="course-date">{new Date(course.createdAt).toLocaleString('fr')}</div></div><div className="course-price">{course.prix?.toLocaleString()} Ar</div></div>)}</div>;
}
...
function VersementsPage() {
  ...
  return <div><div className="card"><div className="card-title">💰 Versements</div></div><div className="card"><div style={{ display: 'flex', gap: 8 }}><input type="number" value={montant} onChange={e => setMontant(e.target.value)} placeholder="Montant" style={{ flex: 1, padding: 10, background: '#252525', border: '1px solid #333', borderRadius: 10, color: '#fff' }} /><button onClick={() => montant && axios.post(`${API}/versements`, { chauffeurId: c?.id, montantVerse: parseFloat(montant) }, { headers: { Authorization: `Bearer ${tk()}` } }).then(() => { setMontant(''); alert('✅ Demande envoyée'); })} className="btn-primary" style={{ width: 'auto' }}>Envoyer</button></div></div>{versements.map((v: any) => <div key={v.id} style={{ background: '#252525', borderRadius: 10, padding: 10, marginBottom: 6, display: 'flex', justifyContent: 'space-between' }}><div><div style={{ fontWeight: 'bold' }}>{v.montantVerse?.toLocaleString()} Ar</div><div style={{ fontSize: 10, color: '#888' }}>{new Date(v.createdAt).toLocaleDateString('fr')}</div></div><span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 20, background: v.statut === 'VALIDE' ? 'rgba(39,174,96,0.2)' : 'rgba(243,156,18,0.2)', color: v.statut === 'VALIDE' ? '#27ae60' : '#f39c12' }}>{v.statut === 'VALIDE' ? 'Validé' : 'En attente'}</span></div>)}</div>;
}
...
function ProfilPage({ onLogout }: { onLogout: () => void }) {
  const c = chauffeur(); const m = moto();
  return <div><div className="card"><div className="card-title">👤 Mon profil</div><div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}><div style={{ width: 60, height: 60, borderRadius: '50%', background: '#DAA520', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, fontWeight: 'bold', color: '#000' }}>{c?.nom?.charAt(0) || '?'}</div><div><div style={{ fontSize: 18, fontWeight: 700 }}>{c?.nom}</div><div style={{ fontSize: 12, color: '#888' }}>{c?.codeAcces}</div></div></div><div style={{ borderTop: '1px solid #333', paddingTop: 12 }}><div className="profil-item"><span style={{ color: '#888' }}>📱 Téléphone</span><span>{c?.telephone || '-'}</span></div><div className="profil-item"><span style={{ color: '#888' }}>🏍️ Moto</span><span>{m?.immatriculation || 'Aucune'}</span></div><div className="profil-item"><span style={{ color: '#888' }}>💰 Solde</span><span style={{ color: '#DAA520', fontWeight: 700 }}>{c?.solde?.toLocaleString() || 0} Ar</span></div></div></div><button onClick={onLogout} style={{ width: '100%', padding: 14, background: 'rgba(239,68,68,0.15)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 12, color: '#f87171', fontWeight: 600, fontSize: 14, cursor: 'pointer', marginTop: 10 }}>🚪 Déconnexion</button></div>;
}
