...
function VersementsPage(){
  ...
  const totalSelected=Array.from(selectedIds).reduce((s,id)=>{const imp=impayes.find((v:any)=>v.id===id);return s+(imp?.reste||0);},0);
      ...
      <div className="card">
        ...
        <div style={{display:'flex',gap:8}}>
          <input type="number" value={montant} onChange={e=>setMontant(e.target.value)}
            ...
            style={{flex:1,padding:12,background:'#252525',border:'1px solid #333',borderRadius:10,color:'#fff',fontSize:14}} />
          <button onClick={envoyer} className="btn-primary" style={{width:'auto',padding:'12px 20px'}}>Envoyer</button>
        </div>
      </div>
        ...
        <div className="modal-overlay" onClick={()=>setShowPayment(false)}>
          <div className="modal-content" onClick={e=>e.stopPropagation()}>
            ...
            <div style={{display:'flex',gap:8}}>
              ...
              <button onClick={()=>{
                axios.post(`${API}/versements/multiple`,{chauffeurId:c?.id,versementIds:Array.from(selectedIds),montantTotal:totalSelected},{headers:{Authorization:`Bearer ${tk()}`}})
                  .then(()=>{setMsg('✅ Versements réglés !');setSelectedIds(new Set());setShowPayment(false);qc.invalidateQueries({queryKey:['versements']});})
                  .catch((err:any)=>setMsg('❌ '+(err.response?.data?.message||'Erreur')));
              }} style={{flex:1,padding:12,background:'#27ae60',border:'none',borderRadius:10,color:'#fff',fontWeight:600,cursor:'pointer'}}>Confirmer</button>
            </div>
          </div>
        </div>
      )}
