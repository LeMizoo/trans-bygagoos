    ...
    <div>
      ...
      {impayes.length>0&&(
        <div className="card" style={{borderLeft:'3px solid #e74c3c'}}>
          <div className="card-title">⚠️ Versements impayés ({impayes.length})</div>
          {impayes.map((v:any)=>(
            <div key={v.id} style={{background:'#252525',borderRadius:10,padding:10,marginBottom:6,display:'flex',justifyContent:'space-between'}}>
              <div><div style={{fontWeight:'bold',color:'#e74c3c'}}>{v.reste?.toLocaleString()} Ar</div><div style={{fontSize:10,color:'#888'}}>{new Date(v.date).toLocaleDateString('fr')} - Dû: {v.montantDu?.toLocaleString()} Ar</div></div>
      ...
      <div className="card">
        <div style={{display:'flex',gap:8}}>
          <input type="number" value={montant} onChange={e=>setMontant(e.target.value)} placeholder="Montant à verser" style={{flex:1,padding:10,background:'#252525',border:'1px solid #333',borderRadius:10,color:'#fff'}}/>
          <button onClick={envoyer} className="btn-primary" style={{width:'auto'}}>Envoyer</button>
        </div>
      </div>
      <div className="card">
