...
function DashboardPage({online}:{online:boolean}){
  ...
  const pointer=useMutation({mutationFn:(type:string)=>axios.post(`${API}/pointages`,{chauffeurId:c?.id,type},{headers:{Authorization:`Bearer ${tk()}`}}),onSuccess:(_,type)=>{const chauffeurData=JSON.parse(localStorage.getItem("chauffeur")||"{}");chauffeurData.statut=type==="ARRIVEE"||type==="REPRISE"?"EN_SERVICE":type==="PAUSE"?"EN_PAUSE":"HORS_SERVICE";localStorage.setItem("chauffeur",JSON.stringify(chauffeurData));const labels:any={ARRIVEE:'✅ Service débuté !',PAUSE:'⏸️ Pause',REPRISE:'🔄 Reprise',FIN_SERVICE:'🏁 Service terminé'};setMsg(labels[type]||'✅ OK');qc.invalidateQueries({queryKey:['dashboard']});setTimeout(()=>setMsg(''),3000);},onError:(err:any)=>setMsg('❌ '+(err?.response?.data?.message||'Erreur'))});
