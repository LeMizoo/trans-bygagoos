...
import {
  ...
  Users, ShoppingCart, MapPin, Receipt, Wallet
} from 'lucide-react';
          ...
          {menuStructure.map((item: any) => {
                ...
                <div key={item.key}>
                  <button 
                    ...
                    className={`w-full flex items-center justify-between px-5 py-2.5 hover:bg-indigo-600 dark:hover:bg-gray-800 transition-colors text-sm ${
                      hasActiveChild ? 'bg-indigo-600/50 dark:bg-gray-800/50' : ''
                    }`}
                  >
                    ...
                    <div className="bg-indigo-800/30 dark:bg-gray-800/30 py-1">
                      {item.children.map((child: any) => (
                        <Link
                          ...
                          className={`flex items-center gap-3 pl-14 pr-5 py-2 text-xs hover:bg-indigo-600 dark:hover:bg-gray-800 transition-colors ${
                            isActive(child.path) ? 'bg-indigo-600 dark:bg-gray-800 font-bold border-l-2 border-l-white' : ''
                          }`}
                        >
            ...
            return (
              <Link
                ...
                to={item.path}
                onClick={() => setSidebarOpen(false)}
                className={`flex items-center gap-3 px-5 py-2.5 hover:bg-indigo-600 dark:hover:bg-gray-800 transition-colors text-sm ${
                  isActive(item.path) ? 'bg-indigo-600 dark:bg-gray-800 font-bold border-l-2 border-l-white' : ''
                }`}
              >
                <item.icon size={17} />
                {item.label}
