...
export class FlottesController {
  ...
  @Post(':id/valider') async valider(@Param('id') id: string) { await this.prisma.flotte.update({ where: { id }, data: { statut: 'ACTIF' } }); return { success: true }; }
  ...
  async restoreAll() {
    ...
    for (const d of flottes) {
      ...
      if (!f) f = await this.prisma.flotte.create({ data: { nom: d.nom, slug: d.nom.toLowerCase().replace(/ /g, '-'), email: d.email, telephone: d.tel, statut: 'ACTIF' } });
